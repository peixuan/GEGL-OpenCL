        ...
        CL_COLOR_EQUAL         == need_babl_out)
    {
        ...
        gegl_buffer_set(dst, dst_rect, out_format, dst_buf,
            src_rect->width * babl_format_get_bytes_per_pixel(out_format));
    ...
    else if (CL_COLOR_CONVERT == need_babl_out)
    {
        ...
        gegl_buffer_set(dst, dst_rect, dst_format, dst_buf,
            src_rect->width * babl_format_get_bytes_per_pixel(dst_format));
        errcode = gegl_clEnqueueUnmapMemObject(gegl_cl_get_command_queue(),
            dst_mem, dst_buf, 
            0, NULL, NULL);
        if (CL_SUCCESS != errcode) CL_ERROR;
